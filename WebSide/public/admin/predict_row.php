...
$payload = [
    'patient_id' => $input['patient_id'] ?? ('DS_' . time()),
...
@file_put_contents($log_dir . '/flask_response.json', date('Y-m-d H:i:s') . ' => ' . $res . "\n", FILE_APPEND);
...
// Save to DB (same pattern as predict_action.php, but source = test_dashboard)
try {
    $pdo = getPDO();

    $ag_db_column = 'AG_Ratio'; // DB column for A/G ratio
    ...
    $stmt->execute([
        ...
        $model_version,
        $record_hash,
        'test_dashboard', // source differs from staff
        json_encode([
...
// Build response to frontend (mirror useful fields, including medical_warning)
$response = [
    'success'        => true,
...
// Optional extra fields, same as predict_action.php
